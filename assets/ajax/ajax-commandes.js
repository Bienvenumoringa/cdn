...
$(document).on('click', 'a[data-role=update]', function(){
    if(! navigator.onLine){
        ...
        if(id.value!="") {
            ...
            let client=$('#'+id).children('td[data-target=client]').text();
            $('#idUp').val(id);
...
for (let i = 0; i < headerRow.cells.length; i++) {
    headerRow.cells[i].textContent = lg.lang_com_header[lag][i];
}

$('form label').find('input, textarea').each(function(index){
    // Vérifiez si l'index existe dans le tableau avant de l'assigner
    if (lg.lang_com_input[lag][index]) {
        $(this).attr('placeholder', lg.lang_com_input[lag][index]);
    }
});

$("form label span").each(function(index) {
    // Vérifiez si l'index existe dans le tableau avant de l'assigner
    if (lg.lang_com_label[lag][index]) {
        $(this).text(lg.lang_com_label[lag][index]);
    }
});