...
$(document).ready(function() {
    $('#send').click(function(e) {
        if (! navigator.onLine) {
            ...
            $.ajax({
                url: '../models/controllers/controller-fournisseur.php',
                type: 'POST',
                data: {
                    action: 'add',
                    name: name,
...
for (let i = 0; i < headerRow.cells.length; i++) {
    headerRow.cells[i].textContent = lg.lang_Four_header[lag][i];
}

$('form label').find('input, textarea').each(function(index){
    // Vérifiez si l'index existe dans le tableau avant de l'assigner
    if (lg.lang_fourn_input[lag][index]) {
        $(this).attr('placeholder', lg.lang_fourn_input[lag][index]);
    }
});

$("form label span").each(function(index) {
    // Vérifiez si l'index existe dans le tableau avant de l'assigner
    if (lg.lang_fourn_label[lag][index]) {
        $(this).text(lg.lang_fourn_label[lag][index]);
    }
});