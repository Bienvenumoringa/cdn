    ...
    $(document).ready(function() {
        $('#send').click(function(e) {
            if (! navigator.onLine) {
                ...
                dp.loaderAdd.html("");
                $.ajax({
                    url: '../models/controllers/controller-groupes.php',
                    type: 'POST',
                    data: {
                        action: 'add',
                        name: name
    ...
    // remplir l'entete de tavbleau
    for (let i = 0; i < headerRow.cells.length; i++) {
        headerRow.cells[i].textContent = lg.lang_Groupe_header[lag][i];
    }

    $('#name').attr('placeholder', lg.lang_goupe_input[lag]);// Completer le placeholder
    $("form label span").eq(0).text(lg.lang_groupe_label[lag]);// Completer d'etiquette
