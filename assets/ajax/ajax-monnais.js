                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response == true){
                            ...
                            dp.loaderUp.removeClass('loader');
                            dp.loaderUp.html(dp.btnUpdateLabel);
                            dp.loaderAdd.html(btnSaveLabel);
                            dp.showMessage(object, content);
                            getData();
                            getCount();
                        } else {
    ...
    $(document).on('click', 'a[data-role=update]', function(){
        if(! navigator.onLine){
            ...
            if(id.value!="") {
                ...
                let operation=$('#'+id).children('td[data-target=operation]').text();
                let taux=$('#'+id).children('td[data-target=taux]').text();
                $('#idUp').val(id);
    ...
    for (let i = 0; i < headerRow.cells.length; i++) {
        headerRow.cells[i].textContent = lg.lang_monnaie_header[lag][i];
    }

    $('form label').find('input, textarea').each(function(index) {
        // Vérifiez si l'index existe dans le tableau avant de l'assigner
        if (lg.lang_monnaie_input[lag][index]) {
            $(this).attr('placeholder', lg.lang_monnaie_input[lag][index]);
        }
    });

    $("form label span").each(function(index) {
        // Vérifiez si l'index existe dans le tableau avant de l'assigner
        if (lg.lang_monnaie_label[lag][index]) {
            $(this).text(lg.lang_monnaie_label[lag][index]);
        }
    });