...
$(document).ready(function() {
    $('#send').click(function(e) {
        if (! navigator.onLine) {
            ...
            dp.loaderAdd.html("");
            $.ajax({
                url: '../models/controllers/controller-paiements.php',
                type: 'POST',
                data: {
                    action: 'add',
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response == true){
                        ...
                        dp.loaderAdd.html(dp.btnSaveLabel);
                        var commande = $('#commandeUp');
                        commande.attr('id', 'commande');
                        dp.showMessage(object, content);
                        hideBtnUp();
                        dp.btnNewClick();
                        getData();
...
$(document).on('click', 'a[data-role=update]', function(){
    if(! navigator.onLine){
        ...
        if(id.value!="") {
            ...
            let commande=$('#'+id).children('td[data-target=commande]').text();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var commandeUp = $('#commande');
                    commandeUp.attr('id', 'commandeUp');
                    $('#commandeUp').html(response)
                    $('#commandeUp').val(commande);// Ici je renvoit l'id de la commande pour que cela selection l'option qui correspond à cette valeur
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    var object = "error";
...
$('form label').find('input, textarea').each(function(index) {
    ...
    if (lg.lang_paie_input[lag][index]) {
        $(this).attr('placeholder', lg.lang_paie_input[lag][index]);
    }
});

$("form label span").each(function(index) {
    // Vérifiez si l'index existe dans le tableau avant de l'assigner
    if (lg.lang_paie_label[lag][index]) {
        $(this).text(lg.lang_paie_label[lag][index]);
    }
});